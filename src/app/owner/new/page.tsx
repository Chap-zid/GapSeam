  ...
  return <main className="page"><div className="shell">
    ...
    <form className="form-layout" onSubmit={submit}><div className="form-card reveal">
      <section className="form-section"><h2>기본 정보</h2><div className="form-grid">
        <div className={`${cls('address')} full`}><label>주소 <span className="required">*</span></label><input name="address" value={form.address} onChange={(e)=>set('address',e.target.value)} placeholder="예: 대전광역시 동구 중앙로 215" required/><p className="help">{form.address.trim()?<>인식된 지역 <b>{extractRegion(form.address)}</b></>:'주변 환경 조회를 위해 건물번호나 지번까지 입력해주세요.'}</p></div>
