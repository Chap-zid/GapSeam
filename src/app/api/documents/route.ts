...
export async function GET(request: Request) {
  ...
  return NextResponse.json({ documents: await listDocuments(session.uid, matchId) });
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ document: await createDocument(file, session.uid, matchId) }, { status: 201 });
  } catch (error) {
