...
export default function AgentPage(){
  ...
  const [analysisSource,setAnalysisSource]=useState<"openai"|"fallback"|null>(null); const [locationSource,setLocationSource]=useState<"vworld"|"fallback"|null>(null);
  ...
  return <main className="agent-page"><div className="shell">
    <div className="agent-head reveal"><div className="agent-icon">{blocked?<ShieldAlert/>:done?<Check/>:<BrainCircuit/>}</div><p className="eyebrow">공간이음 에이전트</p><h1>{blocked?'조금 더 알려주셔야 판단할 수 있어요.':done?'이 공간의 활용 계획을 찾았습니다.':'이 공간의 활용 방법을 찾고 있습니다.'}</h1><p>{subtitle}</p></div>
    <div className="agent-layout"><aside className="timeline-card"><h2>업무 진행 상황</h2><ol className="timeline">{steps.map((label,i)=><li key={label} className={i<step?'done':i===step&&!done?'active':''}><span className="timeline-dot">{i<step&&<Check size={12}/>}</span><span>{label}</span></li>)}</ol></aside>
      <div className="agent-work">
        <section className="discovery"><div className="discovery-head"><Search size={18}/><b>{done?'확인된 공간 정보':'정보를 확인하고 있습니다'}</b>{locationSource&&<span className={`engine-badge ${locationSource==='vworld'?'openai':'fallback'}`}>{locationSource==='vworld'?'VWorld 주변 데이터':'데모 주변 데이터'}</span>}{analysisSource&&!blocked&&<span className={`engine-badge ${analysisSource}`}>{analysisSource==='openai'?'실시간 모델 분석':'안정 모드 분석'}</span>}</div>{step<2?<><div className="skeleton"/><div className="skeleton short"/></>:<div className="finding-grid"><div className="finding"><span>공간 상태</span><b>{blocked?'확인 필요':analysis.condition}</b></div>{step>=3&&<div className="finding"><span>주변 환경</span><b>{blocked?'조회 결과 부족':analysis.surroundingEnvironment}</b></div>}</div>}</section>
