...
export default function MatchChatPage() {
  ...
  return <main className="page match-room-page"><div className="shell">
    ...
    <header className="match-room-head reveal"><div><p className="eyebrow">매칭 전용 협의 공간</p><h1>{space?.address || "공간 연결"}</h1><p>{request?.seekerName || "이용자"} · {request?.purpose || "공간 이용"} · 적합도 {match.score}%</p></div><Link className="button" href={`/matches/${id}/document`}><FileText size={18}/> 공동 문서 열기</Link></header>
    <div className="match-room-layout"><section className="chat-card"><div className="chat-title"><div><MessageCircle/><span><b>실시간 대화</b><small>매칭 참여자만 볼 수 있습니다.</small></span></div><span className="live-chip">실시간 연결</span></div><div className="chat-log">
      ...
      <aside className="match-room-side"><section className="content-card"><div className="card-title"><h2>협의 정보</h2></div><div className="card-pad match-context"><div><Building2/><span>공간<b>{space?.buildingType || "-"} · {space?.area || "-"}㎡</b></span></div><div><Users/><span>이용 목적<b>{request?.purpose || "-"}</b></span></div><div><ShieldCheck/><span>예상 이용료<b>{match.approvedPrice || space?.analysis?.estimatedPrice || "협의"}</b></span></div></div></section><section className="document-bridge"><FileText/><h2>대화 내용을 문서로 구체화하세요.</h2><p>Firestore로 양쪽 기기에 즉시 반영되는 협의 문서를 함께 작성하고, 문서 도우미로 누락된 인허가·안전·일정 항목을 검토할 수 있습니다.</p><Link className="button full-button" href={`/matches/${id}/document`}>공동 문서 작업 시작</Link><Link className="document-advanced-link" href={`/documents?matchId=${encodeURIComponent(id)}`}>DOCX 고급 편집 열기</Link><small>문서 수정은 계약 확정이 아니며 최종 내용은 양측이 직접 확인해야 합니다.</small></section></aside>
    </div>
  </div></main>;
}
