...
function DocumentsContent() {
  ...
  return <main className="document-page"><Script src={`${serverUrl}/web-apps/apps/api/documents/api.js`} onLoad={() => setScriptReady(true)} onError={() => setError("문서 편집 서버가 꺼져 있습니다. Docker를 실행한 뒤 새로고침해주세요.")}/><div className="shell">
    ...
    <div className="document-workspace">
      ...
      <aside className="document-agent"><div className="document-agent-head"><span><Bot size={19}/></span><div><b>AI 문서 검토</b><small>공가 행정·안전 기준</small></div></div><p className="agent-disclaimer">참고용 검토이며 허가 가능성이나 법률 판단을 대신하지 않습니다. 행동 강령 교체는 공가 관련 조항만 다룹니다.</p><div className="document-quick"><button onClick={() => void ask("공가 활용 계획의 권한, 건축·토지, 안전, 행정 절차 누락을 찾아줘")}><ShieldCheck/>필수 절차 점검</button><button onClick={() => void ask("현재 계획에 필요한 공통 문서와 조건부 행정서식을 분류하고 관할 기관 확인사항을 알려줘")}><FileText/>필요 서식 분류</button><button onClick={() => void ask("문서 작성에 빠진 정보만 질문으로 정리해줘")}><Sparkles/>누락 정보 질문</button><button onClick={() => void ask("모호한 표현을 책임 주체와 완료 기준이 보이도록 행정 문체로 고쳐줘")}><FileCheck2/>행정 문체 점검</button><button onClick={() => void ask("문서에서 공가 활용과 직접 관련된 행동 강령 조항만 찾아 교체안을 작성해줘")}><Clipboard/>공가 행동 강령 교체</button></div><label>선택 문장 검토<textarea value={selection} onChange={(e) => setSelection(e.target.value)} placeholder="수정하거나 검토할 문장을 붙여 넣으세요."/></label><label>문서에 질문하기<textarea value={instruction} onChange={(e) => setInstruction(e.target.value)} placeholder="예: 지자체 상담 전에 무엇을 확인해야 해?"/></label><button className="button document-ask" disabled={!current || reviewing || (!instruction.trim() && !selection.trim())} onClick={() => void ask(instruction || "공가 활용 계획의 누락과 행정 절차를 점검해줘")}>{reviewing ? <><LoaderCircle className="spin"/> 검토 중...</> : "에이전트에게 묻기"}</button>{review && <div className="document-review"><p>{review.summary}</p>{review.findings.map((item, index) => <div className="review-finding" key={`${item.title}-${index}`}><span>{item.level}</span><b>{item.title}</b><small>{item.detail}</small></div>)}{!!review.requiredDocuments.length && <div className="document-guides"><b>필요 서식</b>{review.requiredDocuments.map((item) => <div key={item.name}><span>{item.status}</span><strong>{item.name}</strong><small>{item.reason}</small></div>)}</div>}{!!review.questions.length && <div className="document-questions"><b>확인 질문</b><ol>{review.questions.map((question) => <li key={question}>{question}</li>)}</ol></div>}{review.rewrite && <div className="review-rewrite"><b>{instruction.includes("행동 강령") ? "행동 강령 교체안" : "수정 제안"}</b><p>{review.rewrite}</p><button type="button" className="copy-rewrite" onClick={() => void copyRewrite()}><Clipboard size={14}/>{instruction.includes("행동 강령") ? "교체안 복사" : "수정안 복사"}</button></div>}<b>다음 확인</b><ol>{review.nextActions.map((action) => <li key={action}>{action}</li>)}</ol></div>}</aside>
    </div>
  </div></main>;
}
